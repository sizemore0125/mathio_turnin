...
import ShapeDrag from "./shapedrag";
import { Helmet } from "react-helmet";
import axios from "axios";

const ShapeProblem = ({ match, history }) => {
  ...
  return (
    <React.Fragment>
      ...
      <div className="heading">
        ...
        <h2 className="shape-instructions">
          Fill each box with the same number of circles.
        </h2>
        <button
          className="submit"
          type="submit"
          onClick={() => {
            handleSubmit(answer, match);
          }}
        >
          Submit
        </button>
...
const handleSubmit = async (answer, match) => {
  ...
  thisProblem.answer = JSON.stringify(answer);
